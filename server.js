...
app.post('/api/upload', upload.single('video'), async (req, res) => {
    ...
    try {
        ...
        // Usando copyFile e unlink em vez de rename para maior resiliência
        ...
        if (error instanceof multer.MulterError) {
            if (error.code === 'LIMIT_FILE_SIZE') {
                return res.status(400).json({ message: `O arquivo é muito grande. O tamanho máximo permitido é de 15 MB.` });
            }
